...
export async function generateMetadata({
  ...
  return buildLocalizedMetadata({
    locale,
    path: "/me/orders",
    title: t("meta.ordersTitle", locale),
    description: t("meta.ordersDescription", locale),
    noindex: true,
  });
}
...
export default async function MyOrdersPage({
  ...
  if (!token) redirect(`${lp("/login")}?next=${encodeURIComponent(lp("/me/orders"))}`);
  ...
  try {
    ...
    if (err instanceof BackendUnauthorized)
      redirect(`${lp("/login")}?next=${encodeURIComponent(lp("/me/orders"))}`);
    throw err;
  }
  ...
  return (
    <main className="min-h-screen pb-20">
      ...
      <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        <h1 className="mt-3 mb-1 text-2xl font-black">{tr("me.ordersHeading")}</h1>
        <p className="mb-6 text-sm text-slate-400">{tr("me.ordersSubtext")}</p>

        {items.length === 0 && (
          <Card>
            <p className="text-sm text-slate-300">{tr("me.ordersEmpty")}</p>
          </Card>
        )}
