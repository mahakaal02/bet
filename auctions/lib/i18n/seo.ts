/**
 * Localized SEO metadata helper (PR-AUCTIONS-I18N).
 *
 ...
 *      `noindex: true` for authenticated surfaces (profile, me/*,
 *      notifications, etc.) so private content doesn't end up in search.
 *
...
export interface LocalizedMetadataInput {
   ...
   * "/auctions" / "/auctions/some-id" for sub-paths.
   */
  path: string;
  /**
   ...
   * serve user-specific content (profile, me/*, notifications,
   * watchlist) — they'd either 401 the crawler or pollute the index
   * with empty pages.
   */
  noindex?: boolean;
}
 ...
 *       locale,
 *       path: "/auctions",
 *       title: t("auction.heading", locale),
