/**
 * Analytics-attribution helpers for the i18n layer (PR-AUCTIONS-I18N).
 *
 ...
 *      `/auctions?utm_campaign=launch` to `/pt/auctions?utm_campaign=launch`,
 *      or when the language switcher swaps `/pt/auctions?ref=alice` to
 *      `/en/auctions?ref=alice`, the marketing/referral state must
 ...
 *
 *   appendTrackingParams("/en/auctions", { utm_source: "twitter" })
 *     → "/en/auctions?utm_source=twitter"
 *   appendTrackingParams("/en/auctions?utm_id=x", { utm_id: "y" })
 *     → "/en/auctions?utm_id=x"           ← target wins; idempotent
 */
 ...
 *
 *   const target = withPreservedParams("/en/auctions", currentParams);
 *
 * For UX consistency we currently preserve ALL params, not just
 * tracking ones — a user who switches language while looking at
 * `/pt/auctions?tab=LIVE` expects to land on
 * `/en/auctions?tab=LIVE`. The tracking-only API
 ...
 * `Locale` shape.
 *
 *   analytics.track('bid_placed', { locale: localeDimension(locale) });
 */
