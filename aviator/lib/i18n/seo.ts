/**
 * Localized SEO metadata helper (PR-AVIATOR-I18N).
 *
 ...
 *      `noindex: true` for authenticated surfaces (profile, withdraw,
 *      notifications) so private content doesn't end up in search.
 *
...
export interface LocalizedMetadataInput {
   ...
   * "/fairness" / "/profile" for sub-paths.
   */
  path: string;
  /**
   ...
   * serve user-specific content (profile, withdraw, notifications) —
   * they'd either 401 the crawler or pollute the index with empty
   * pages.
   */
  noindex?: boolean;
}
