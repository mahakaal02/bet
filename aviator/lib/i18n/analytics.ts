/**
 * Analytics-attribution helpers for the i18n layer (PR-AVIATOR-I18N).
 *
 ...
 *      `/?utm_campaign=launch` to `/pt/?utm_campaign=launch`,
 *      or when the language switcher swaps `/pt/profile?ref=alice` to
 *      `/en/profile?ref=alice`, the marketing/referral state must
 ...
 *
 *   appendTrackingParams("/en/profile", { utm_source: "twitter" })
 *     → "/en/profile?utm_source=twitter"
 *   appendTrackingParams("/en/profile?utm_id=x", { utm_id: "y" })
 *     → "/en/profile?utm_id=x"           ← target wins; idempotent
 */
 ...
 *
 *   const target = withPreservedParams("/en/profile", currentParams);
 *
 * For UX consistency we currently preserve ALL params, not just
 * tracking ones — a user who switches language while looking at
 * `/pt/fairness?round=42` expects to land on
 * `/en/fairness?round=42`. The tracking-only API
 ...
 *
 * Optionally accepts a hash fragment (e.g. "#section") which is
 * appended after the query string. Pass `null`/`undefined` to skip
 * — typical for non-client-side callers that don't have a
 ...
 * `Locale` shape.
 *
 *   analytics.track('bet_placed', { locale: localeDimension(locale) });
 */
